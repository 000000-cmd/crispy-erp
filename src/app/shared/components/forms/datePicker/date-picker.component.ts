    ...
    <div class="relative w-full">
      <div class="flex flex-col gap-2">
        ...
        <div (click)="toggleCalendar()"
             class="w-full h-12 flex items-center justify-between px-2.5 gap-2  
                    backdrop-blur-sm border-2 rounded-md cursor-pointer transition-all duration-300 
                    hover:bg-white dark:hover:bg-[#303a4b] bg-white/50 dark:bg-gray-700/85 border-slate-600  "
          ...
          <span class="grow text-left" [ngClass]="displayValue ? 'text-gray-800 dark:text-gray-300' : 'text-gray-400'">
      ...
      <div class="datepicker-panel dark:bg-(--foreground-dark)" [class.datepicker-panel-hidden]="!isOpen">
        <div class="flex items-center justify-between mb-4">
          <button (click)="navigate(-1)" type="button" class="p-2 rounded-full dark:bg-(--foreground-dark)  border-slate-200 text-gray-900 dark:text-gray-200 hover:bg-gray-100 cursor-pointer">
            <lucide-angular [img]="ChevronLeft" class="w-5 h-5"></lucide-angular>
          </button>
          <div class="flex gap-2">
            @if (currentView !== 'years') {
              <button (click)="setView('months')" type="button" class="font-bold text-gray-800 dark:text-gray-300 p-1 rounded-md hover:bg-gray-100 cursor-pointer transition-colors">{{ monthNames[currentMonth] }}</button>
            }
            <button (click)="setView('years')" type="button" class="font-bold text-gray-800     dark:text-gray-300 p-1 rounded-md hover:bg-gray-100 cursor-pointer transition-colors">{{ currentYear }}</button>
          </div>
          <button (click)="navigate(1)" type="button" class="p-2 rounded-full text-gray-900 dark:text-gray-200 hover:bg-gray-100 cursor-pointer">
            ...
            <div class="grid grid-cols-7 gap-1 text-center">
              ...
              @for (day of calendarDays; track $index) {
                <div (click)="selectDate(day)"
                     ...
                     [ngClass]="{'bg-purple-600 text-white hover:bg-purple-700': isSelected(day), 'hover:bg-gray-100': day && !isSelected(day), 'text-gray-800 dark:text-gray-300': day, 'cursor-default': !day}">
                  {{ day }}
                </div>
              }
            </div>
          ...
          @case ('months') {
            <div class="grid grid-cols-3 gap-2">
              @for (month of monthNames; track month; let i = $index) {
                <div (click)="selectMonth(i); $event.stopPropagation()" class="p-3 rounded-lg text-gray-800 text-sm text-center font-semibold cursor-pointer transition-colors"
                 [ngClass]="i === currentMonth ? 'bg-purple-600 text-white' : 'hover:bg-gray-100'">
                  {{ month.substring(0, 3) }}
                </div>
              }
            </div>
          ...
          @case ('years') {
            <div class="grid grid-cols-3 gap-2">
              @for (year of yearRange; track year) {
                <div (click)="selectYear(year); $event.stopPropagation()" class="p-3 text-gray-800 rounded-lg text-sm text-center font-semibold cursor-pointer transition-colors"
                 [ngClass]="year === currentYear ? 'bg-purple-600 text-white' : 'hover:bg-gray-100'">
                  {{ year }}
                </div>
              }
            </div>
