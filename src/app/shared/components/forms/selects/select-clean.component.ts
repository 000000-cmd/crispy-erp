...
export interface ListItem {
  id: string;
  code: string; //El valor el cual se usa
  name: string; //El valor el cual se muestra
  order: number;
...
@Component({
  selector: 'select-clean',
  standalone: true,
  imports: [CommonModule, LucideAngularModule, BaseInputLabel],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => SelectCleanComponent),
      multi: true,
    },
  ],
  template: `
    <div class="w-full flex flex-col gap-2">
      ...
      <div class="relative">
        <button [id]="_value" type="button" (click)="toggle()" (blur)="onTouched()" [disabled]="disabled"
                class="cursor-pointer flex w-full items-center justify-between gap-2 rounded-md border-2 border-slate-600
                 bg-white dark:bg-[#364153] px-3 py-2 text-sm outline-none h-10 
                 focus-visible:ring-2 focus-visible:ring-purple-500 disabled:cursor-not-allowed disabled:opacity-50">
        ...
        <div role="listbox"
             class="absolute mt-1 w-full rounded-md border-2 border-slate-600 bg-white dark:bg-[#364153] shadow-md z-50 max-h-60 overflow-y-auto transition-all duration-200 ease-out origin-top"
          ...
          @for (option of options; track option.code) {
            <div (click)="selectItem(option)" class="flex items-center gap-2 cursor-pointer px-2 py-3 text-sm hover:bg-purple-100 dark:hover:bg-purple-800" [class.font-semibold]="option.code === _value" [id]="option.code">
              {{ option.name }}
            </div>
          }
        </div>
...
export class SelectCleanComponent implements ControlValueAccessor {
